...
begin
  ...
  if to_regclass('public.player_alliance_profiles') is not null then
    ...
    if not exists (select 1 from pg_policies where schemaname='public' and tablename='player_alliance_profiles' and policyname='pap_select_own') then
      execute $p$
        ...
        for select
        using (
          exists (select 1 from public.players me where me.auth_user_id = auth.uid() and me.id = player_alliance_profiles.player_id)
        )
      $p$;
    end if;

    if not exists (select 1 from pg_policies where schemaname='public' and tablename='player_alliance_profiles' and policyname='pap_insert_own') then
      execute $p$
    ...
    if not exists (select 1 from pg_policies where schemaname='public' and tablename='player_alliance_profiles' and policyname='pap_update_own') then
      execute $p$
    ...
    if not exists (select 1 from pg_policies where schemaname='public' and tablename='player_alliance_profiles' and policyname='pap_delete_own') then
      execute $p$
  ...
  if to_regclass('public.player_hqs') is not null then
    ...
    if not exists (select 1 from pg_policies where schemaname='public' and tablename='player_hqs' and policyname='phq_insert_own') then
      execute $p$
    ...
    if not exists (select 1 from pg_policies where schemaname='public' and tablename='player_hqs' and policyname='phq_update_own') then
      execute $p$
    ...
    if not exists (select 1 from pg_policies where schemaname='public' and tablename='player_hqs' and policyname='phq_delete_own') then
      execute $p$
