...
const [alliances, setAlliances] = useState<AllianceRow[]>([]);
  ...
  // ✅ New: create player inputs
  const [newPlayerName, setNewPlayerName] = useState("");
  const [newPlayerRole, setNewPlayerRole] = useState<MembershipRow["role"]>("member");
  ...
  // ✅ New: Create player (game name) and optionally add to selected alliance
  const createPlayer = async () => {
    const name = newPlayerName.trim();
    if (!name) return alert("Game Name required.");

    // Try insert with game_name, fallback to name if schema differs
    let createdId: string | null = null;

    const resA = await supabase.from("players").insert({ game_name: name }).select("id").single();
    if (!resA.error) {
      createdId = resA.data?.id ?? null;
    } else {
      const msg = (resA.error.message || "").toLowerCase();
      if (msg.includes("column") && msg.includes("game_name")) {
        const resB = await supabase.from("players").insert({ name }).select("id").single();
        if (resB.error) {
          console.error(resB.error);
          alert(resB.error.message);
          return;
        }
        createdId = resB.data?.id ?? null;
      } else {
    ...
    if (!createdId) return alert("Failed to create player.");

    // Optionally add membership immediately
    if (newAutoAdd && selectedAlliance) {
      const { error } = await supabase.from("player_alliances").insert({
        player_id: createdId,
        alliance_code: selectedAlliance,
        role: newPlayerRole,
    ...
    // convenience: set dropdowns to the new player
    setAddPlayerId(createdId);
    setLinkPlayerId(createdId);
  };
    ...
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: "grid", gap: 12, maxWidth: 900 }}>
        ...
        <div style={{ border: "1px solid #333", borderRadius: 10, padding: 12 }}>
          <h3 style={{ marginTop: 0 }}>Create Player</h3>

          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "end" }}>
            <label style={{ display: "grid", gap: 6 }}>
              <span>Game Name</span>
              <input value={newPlayerName} onChange={(e) => setNewPlayerName(e.target.value)} placeholder='Example: "Seven"' />
            </label>

            <label style={{ display: "grid", gap: 6 }}>
              <span>Role (if auto-add)</span>
            ...
            <label style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 4 }}>
              <input
                ...
                disabled={!selectedAlliance}
              />
              Auto-add to selected alliance
            </label>
          ...
          <div style={{ marginTop: 8, fontSize: 12, opacity: 0.75 }}>
            If an alliance is selected and “Auto-add” is checked, the new player is immediately added to that alliance with the chosen role.
          </div>
        </div>
