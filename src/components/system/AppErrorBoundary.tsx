...
function CrashOverlay(props: { error: any; componentStack?: string }) {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      try {
        theme = localStorage.getItem("sad_theme");
      } catch {
        theme = null;
      }
    ...
    >
      <div className="zombie-card" style={{ width: "min(980px, 96vw)", padding: 16 }}>
        ...
        <div style={{ opacity: 0.9, marginBottom: 10 }}>
          A runtime error occurred. Use the buttons below to copy a crash bundle and recover.
        </div>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          <button
            ...
            onClick={() => {
              const txt = JSON.stringify(bundle || { loading: true }, null, 2);
              navigator.clipboard?.writeText(txt);
              window.alert("Copied crash bundle JSON to clipboard.");
            }}
            disabled={loading}
          >
            🧾 Copy Crash Bundle
          ...
          <button className="zombie-btn" onClick={() => (window.location.href = "/status")}>
            🧪 Go to /status
          </button>

          <button className="zombie-btn" onClick={() => window.location.reload()}>
            🔄 Reload
          </button>
        </div>
...
export default class AppErrorBoundary extends React.Component<Props, State> {
  ...
  componentDidCatch(error: any, info: any) {
    // Keep stack available, and log for devtools
