...
export default function AllianceDefaultReminderChannelPanel() {
  ...
  async function load() {
    ...
    // simple permission: Owner/Admin always can. For R5/R4 we'll rely on RLS + UI still works if allowed.
    try {
      const me = await supabase.auth.getUser();
      setCanManage(!!me.data?.user);
    } catch {}
  ...
  return (
    <div className="zombie-card" style={{ padding: 12, borderRadius: 16 }}>
      ...
      <div style={{ marginTop: 8, opacity: 0.75, fontSize: 12 }}>
        Discord → Settings → Advanced → Developer Mode → right-click a channel → Copy Channel ID.
      </div>

      <div style={{ marginTop: 10, display: "grid", gap: 10 }}>
        <label style={{ display: "grid", gap: 6 }}>
          <div style={{ fontWeight: 900, fontSize: 12, opacity: 0.9 }}>Alerts Channel ID</div>
          <input value={alertsChannelId} onChange={(e) => setAlertsChannelId(e.target.value)} placeholder="(optional)" />
        </label>

        <label style={{ display: "grid", gap: 6 }}>
          <div style={{ fontWeight: 900, fontSize: 12, opacity: 0.9 }}>Announcements Channel ID</div>
          <input value={announcementsChannelId} onChange={(e) => setAnnouncementsChannelId(e.target.value)} placeholder="(optional)" />
        </label>

        <label style={{ display: "grid", gap: 6 }}>
          <div style={{ fontWeight: 900, fontSize: 12, opacity: 0.9 }}>Reminder Channel ID</div>
          <input value={reminderChannelId} onChange={(e) => setReminderChannelId(e.target.value)} placeholder="(optional)" />
        </label>
      </div>

      <div style={{ marginTop: 10, display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
        <button type="button" onClick={() => void save()} disabled={!canManage}>Save</button>
