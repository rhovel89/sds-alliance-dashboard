...
function parse(body: string): { text: string; atts: Att[] } {
  ...
  for (const ln of lines) {
    ...
    if (m) {
      atts.push({ path: m[1], name: m[2], mime: m[3] });
    } else {
      kept.push(ln);
    }
  }
      ...
      {parsed.atts.length ? (
        <div style={{ marginTop: 10, borderTop: "1px solid rgba(255,255,255,0.12)", paddingTop: 10 }}>
          ...
          <div style={{ marginTop: 8, display: "flex", gap: 10, flexWrap: "wrap" }}>
            {parsed.atts.map((a) => (
              <div key={a.path} style={{ border: "1px solid rgba(255,255,255,0.12)", borderRadius: 14, padding: 10, width: 220 }}>
                <div style={{ fontWeight: 900, fontSize: 12, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{a.name}</div>
                ...
                <div style={{ marginTop: 8, display: "flex", gap: 8, justifyContent: "flex-end" }}>
                  <button type="button" onClick={() => void open(a.path)}>Open</button>
                </div>
              </div>
            ))}
          </div>
