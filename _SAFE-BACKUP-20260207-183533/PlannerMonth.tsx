...
function expandEventsForRangeWithExceptions(
  ...
  // Track overrides so we can add moved occurrences even if original date isn't in-range
  ...
  for (const ev of baseEvents) {
    ...
    function applyExceptionOrPush(baseIso: string) {
      ...
      // fallback: if malformed exception, just show base
      if (isIsoInRange(baseIso, rs, re)) push(baseIso, makeOccurrence(ev, baseIso));
    }
  ...
  // Add overrides whose original date wasn't in-range but moved date is in-range
  for (const ex of overrideList) {
    ...
    // Prevent duplicates if it was already added through normal iteration
  ...
  // Sort each day by time for agenda niceness
...
export function PlannerMonth({ initialEvents }: Props) {
  ...
  // Load exceptions whenever events list changes
  useEffect(() => {
    (async () => {
      try {
  ...
  async function handleDelete() {
    ...
    try {
      ...
      // allow local removal even if remote fails
    }
  ...
  async function handleSkipOccurrence(occIso: string) {
    ...
    try {
      const ex = await upsertExceptionSkip(editingEvent.id, occIso);
      setExceptions((prev) => {
      ...
      // if it fails, do nothing (no silent UI lies)
    }
  }
