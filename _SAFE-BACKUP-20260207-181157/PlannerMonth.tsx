...
import { listUserAlliances, listVisibleEvents, upsertEvent, type UserAlliance } from './eventsStore';
...
export function PlannerMonth({ initialEvents }: Props) {
  ...
  // Load events (Supabase first, fallback to localStorage)
  useEffect(() => {
    (async () => {
      try {
  ...
  // Keep localStorage mirrored for instant refresh
  useEffect(() => {
  ...
  async function handleSave(draft: Draft) {
    ...
    try {
      ...
      // If Supabase fails, we still save locally so nothing feels broken
      setEvents((prev) => {
