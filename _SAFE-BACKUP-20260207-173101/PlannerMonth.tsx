...
function makeOccurrence(ev: CalendarEvent, iso: string): Occurrence {
  return {
    ...
    sourceId: ev.id,
    occurrenceDate: iso,
    // Unique id per occurrence so lists are stable
    id: ev.id + '@' + iso,
  };
}
...
function expandEventsForRange(
  ...
  for (const ev of baseEvents) {
    ...
    if (freq === 'monthly') {
      // Simple, predictable monthly rule (safe baseline):
      // repeats on the same day-of-month as the startDate (e.g., the 15th).
      // (We can expand this later to "nth weekday" if you want.)
      const targetDay = start.getDate();

      // Iterate each day in range and match day-of-month
      for (let d = startOfDay(iterStart); d.getTime() <= re.getTime(); d = addDays(d, 1)) {
        if (d.getTime() < start.getTime()) continue;
  ...
  // Sort occurrences per day by time (optional, but helps agenda)
...
export function PlannerMonth({ initialEvents }: Props) {
  ...
  // Save base rule events (not occurrences)
  useEffect(() => {
    ...
    <div className="v2-planner">
      <div className="v2-planner__main">
        ...
        <div className="v2-grid">
          {grid.map((cell) => {
            const evCount = (occurrencesByDay.get(cell.iso) || []).length;
            return (
              <DayCell
                key={cell.iso}
