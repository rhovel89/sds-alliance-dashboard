-- Queue a Discord WEBHOOK send (per-alliance “channels” via webhooks)
-- This matches your existing discord_send_queue columns:
--   state_code, alliance_code, channel_name, message, payload, status, send_at, created_by
--
-- Worker behavior:
--   payload.kind = 'discord_webhook'
--   channel_name contains either:
--     - a webhook id, OR
--     - 'default:<kind>' (e.g. default:achievements) which your worker resolves per alliance
...
begin
  ...
  -- Use your existing permission helper.
...
-- Helpful view (optional): makes it easy to query “kind/target/channel_id” without errors
...
select
  ...
  q.state_code,
  q.alliance_code,
  coalesce(q.payload->>'kind','') as kind,
  ''::text as target,
  coalesce(nullif(q.channel_name,''), '') as channel_id,
  left(coalesce(q.message,''), 220) as message_preview
from public.discord_send_queue q;
