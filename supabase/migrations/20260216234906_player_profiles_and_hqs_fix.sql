...
-- RLS: player can edit own; alliance owner/r4/r5 and app admin can view.

create extension if not exists pgcrypto;

-- Helper updated_at function (only if you don't already have one with this exact name)
do $$
begin
...
-- Ensure profile_id exists, attempting to rename older column names if present
do $$
begin
  if not exists (
...
do $$
begin
  -- SELECT profiles: player OR alliance owner/r4/r5 OR app admin
  if not exists (select 1 from pg_policies where schemaname='public' and tablename='player_alliance_profiles' and policyname='player_profiles_select') then
    create policy player_profiles_select
      ...
      using (
        ...
        or sa_is_alliance_role(alliance_code, array['owner'::text,'r5'::text,'r4'::text])
        or exists (
  ...
  -- WRITE profiles: only player OR app admin
  ...
  -- SELECT HQs: player OR alliance owner/r4/r5 OR app admin (through profile)
    ...
    create policy player_hqs_select
      ...
      using (
        ...
        or exists (
          ...
          from public.player_alliance_profiles ap
          where ap.id = player_hqs.profile_id
            and sa_is_alliance_role(ap.alliance_code, array['owner'::text,'r5'::text,'r4'::text])
        )
      );
  end if;

  -- WRITE HQs: only player OR app admin (through profile)
