...
serve(async (req) => {
  try {
    ...
    // Permission gate (Owner/AppAdmin only)
    ...
    if (!isAdmin && !isOwner) {
      return json(403, { ok: false, error: "Forbidden (Owner/AppAdmin only)" });
    }
    ...
    if (mode !== "bot") return json(400, { ok: false, error: "Only mode=bot supported for now" });
    ...
    if (content.length > 2000) return json(400, { ok: false, error: "Content too long (Discord limit 2000 chars)" });
    ...
    if (dryRun) {
      return json(200, {
        ...
        asUser: { id: user.id, email: user.email || null },
